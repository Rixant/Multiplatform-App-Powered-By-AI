...
function fromTexture(gl, texture, sourceDims, targetShape, options = {}) {
    ...
    const targetDepth = glCapabilities.canDownloadFromRGBTexture ? originalTargetDepth : 4;
    const _sourceDims = {
    ...
    const resizedTexture = camera_webgl_util_1.runResizeProgram(gl, texture, _sourceDims, _targetShape, alignCorners, interpolation);
    const textureData_ = camera_webgl_util_1.downloadTextureData(gl, resizedTexture, _targetShape);
    let textureData;
    ...
    return tf.tensor3d(textureData, [_targetShape.height, _targetShape.width, originalTargetDepth], 'int32');
}
...
function renderToGLView(gl, texture, size, flipHorizontal = true) {
    ...
    camera_webgl_util_1.drawTexture(gl, texture, _size, flipHorizontal);
}
